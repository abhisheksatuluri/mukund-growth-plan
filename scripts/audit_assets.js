// Silent audit of code/assets/ images using Gemini vision.
...
const ASSETS_DIR = path.resolve('./code/assets');
const OUT_DIR = path.resolve('./output/data');
const OUT_FILE = path.join(OUT_DIR, 'image_audit.json');
const SUMMARY_FILE = path.join(OUT_DIR, 'image_audit_summary.json');
...
async function auditOne(filename, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isRetryable = e.message.includes('503') || e.message.includes('429') || e.message.includes('500');
...
async function main() {
  ...
  const summary = {
    ...
    avg_premium_score: +(results.filter((r) => r.premium_feel_score_0_10).reduce((s,r) => s + r.premium_feel_score_0_10, 0) / results.filter(r => r.premium_feel_score_0_10).length).toFixed(1),
    errored: results.filter((r) => r.error).map((r) => r.filename),
    ...
    needs_regen: results.filter((r) => r.needs_regen).map((r) => ({ file: r.filename, score: r.premium_feel_score_0_10, use: r.best_use, reason: r.regen_reason })),
  };
  results.forEach((r) => {
